...
public final class MesosUtil {
  ...
  public static boolean doesOfferMatchResources( Resources resources, List<Resource> offerResources, List<Long> otherRequestedPorts) {
    ...
    double numCpus = getNumCpus(offerResources, Optional.<String>absent());

    if (numCpus < resources.getCpus()) {
      return false;
    }

    double memory = getMemory(offerResources, Optional.<String>absent());

    if (memory < resources.getMemoryMb()) {
      return false;
    }

    double disk = getDisk(offerResources, Optional.<String>absent());
